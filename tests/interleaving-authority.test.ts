 ...
 * 根拠にした規定:
 *
 * - 1927 行「別の値の反映が要求されたら、**後から来た値が載ります**（後勝ち）。
 *   先の書き込みの完了を待ってから改めて反映するため、**最後に供給された値が
 *   画面とバインドデータの双方に残ります**」
 * - 1928 行「**利用者が入力を確定したら、待っていた書き込みは行いません**。反映を
 *   要求した時点より後の編集は…保護します（**要求より前の編集は、明示的な供給が
 *   権威なので上書きします**）」
 * - 1805 行「外部からの書き換えは『明示的な値の供給』として扱い、配下の入力欄から
 *   ユーザー編集の印を解除します。**利用者が編集した欄も、書き換えた値へ更新され
 *   ます**」
 * - 1910 行「`Core.setBindingData()` の直接呼び出し」は編集の印を解除する供給
 * - 3704 / 3705 行 リセットは DOM の値を既定値へ戻し、バインドデータを初期
...
/** 初期の `data-bind` 宣言（リセットの戻り先。仕様 3705 行） */
...
describe('値の供給と編集の割り込み', () => {
  ...
  for (const settleBetween of [false, true]) {
    ...
    describe(`供給 → 供給（後勝ち・仕様 1927 行）／${suffix}`, () => {
    ...
    describe(`供給 → 編集（編集を保護・仕様 1928 行）／${suffix}`, () => {
    ...
    describe(`編集 → 供給（供給が権威・仕様 1928 / 1805 行）／${suffix}`, () => {
      for (const supply of SUPPLIES) {
