 ...
 * 期待値は仕様書から取っています。
 *
 * - 仕様「反映待ちの間に起きた変化」「最後に供給された値が残る」（後勝ち）
 * - 仕様「ユーザー編集と宣言バインドの権威」「`change` / `input` による双方向バインディングのコミット」は値の
 *   供給ではないため、確定した編集を持つ宛先へは通番によらず適用しない
 * - 仕様「ユーザー編集と宣言バインドの権威」「解除の範囲は共通の規則で決まります」
 *
...
describe('バインドデータの宛先の粒度', () => {
  ...
  it.each(NESTED_EACH_DECLARATIONS)(
    ...
    async (_label, markup) => {
      ...
      // 宛先として記録され、**誰も供給していない混ざり方**が残る（仕様
      // 「`Core.setBindingData(element, data, options?)`」「反映待ちの間に起きた変化」）。
      //
