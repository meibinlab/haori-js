        ...
        chain = chain.then(() =>
          Core.updateRowFragment(
            ...
            Core.repositionEachRow(
              ...
              // 「明示的な値の供給」なら、その供給が自分の操作の時点を基準に解除
              // を済ませている（`Core.setBindingData()`・`Form.reset()`・行への
              // `data-{event}-copy`）。差分更新が重ねて解除すると、供給ではない
              // 更新（双方向コミット・`data-url-param` の再評価・`data-poll` の
              // 応答・行への `data-{event}-bind`）でも解除してしまい、行内の入力
              // を編集した瞬間に**そのコミット自身が保護を外す**。宣言バインドの
              // 評価が要素データと食い違う構成（数値の `id` と文字列の収集値を
              // 厳密比較する `data-attr-selected` など）では、確定した編集が画面
              // と収集値から静かに消える（仕様「ユーザー編集と宣言バインドの
              // 権威」）。
