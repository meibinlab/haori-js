...
`,
  ...
  constructor(
    ...
    // で行う。順序の逆転が問題になるのは「外部が書き換えた直後に利用者が操作した」
    // 場合だけなので、対象をそこへ絞る。起点が DOM イベントでない手続き
    // （`data-load-*`、`data-poll-*`、`data-intersect-*`、シグネチャ算出）は発番だけ
    // を行う。
    //
    // なお「バインドワークの内部で引き取ると実行中のワークへ再入する」ことへの備えは
    // `nextOperationSequence()` の側にある（実行中なら引き取らない）。ここでの絞り込み
    // は意味づけの問題で、再入の防止に依存しているわけではない。
