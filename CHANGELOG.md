...
- `tests/import-scope.test.ts` を追加（7 件、いずれも修正前に失敗）。祖先の `data-bind` の継承、断片の中の `data-if` の評価（真・偽の両方）、取り込み先要素自身の `data-bind` の参照、入れ子の取り込み、`data-each` の行の中での行スコープの参照、再取り込みで古い子がフラグメント木へ残らないことを検証する。
...
- `playwright/import-scope.spec.cjs` と repro ページを追加（2 件、うち 1 件は修正前に失敗）。実ブラウザで、初期読み込みの継承（祖先・取り込み先自身の両方、`data-if` の真偽を含む）と、初期化後に URL が確定した取り込みの評価を確認する。監視の通知順序は実ブラウザでしか確かめられないため、jsdom と両方で押さえている。
- `tests/row-scope-identifier-diagnostics.test.ts` を追加（5 件、うち 2 件は修正前に失敗）。未マウント走査を経ても行スコープの名前を報告しないこと、その構成で行と行の中の `data-fetch` が正しく評価されること、応答のバインド先を取り違えた宣言は従来どおり報告すること、本番（開発モード無効）では記録しないことを検証する。
