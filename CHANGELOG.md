...
- **バインドに無い識別子が `window` 経由で解決される不具合を修正**。評価関数のスコープチェーンがグローバルまで届いていたため、未解決参照になるべき識別子が「値のある参照」に化けていた。`id="agencyCode"` の要素があると `{{agencyCode}}` がその要素オブジェクトへ解決され（HTML の named access on window）、`data-attr-value` に書くと入力欄へ `[object HTMLInputElement]` が書き込まれて `required` の必須検証も通ってしまう状態だった。`{{name}}` `{{status}}` `{{length}}` `{{origin}}` のような `window` の既存プロパティも同様に解決され、`unresolvedReference` が立たないため `data-if` の判定や `data-fetch` の未実行判定も効いていなかった。式に現れる識別子のうちバインドに無いものを評価関数のスコープ内へ宣言して遮蔽し、参照された場合は従来の「バインドに無いキーを参照した」場合と同じ扱い（`ReferenceError` → 未解決参照、`error` ログに識別子名を明示）にした。`?.` / `??` / `||` / `&&` を含む式で `undefined` として評価を続ける挙動、短絡評価や三項演算子で参照されなかった識別子を未解決にしない挙動はいずれも変わらない。`Math` / `JSON` / `Array` などの標準組み込みは従来どおり参照できる（仕様書に一覧を明記）。**意図せずグローバルへ解決されていた式に依存している画面では、その参照が未解決参照になる**ため、`data-bind` でキーを宣言するか `??` でフォールバックを与える必要がある。同梱デモにもこの依存が多数あり（`window.username = ...` のようなグローバル変数の定義や、`<div id="result">` の内側での `{{result}}` 参照）、あわせて修正した（下記 Docs 参照）。`{{result}}` の例は、修正前は id 由来の named access で div 要素へ解決され、画面に `[object HTMLDivElement]` と表示されていた。
...
- `_poll` の参照方法を明記。注入先は `_poll` を参照する要素の祖先にする必要があること（式は祖先方向へ辿って解決するため）、初期表示時点では `_poll` が存在しないため `data-bind` で `"_poll": null` を宣言して `_poll?.xxx` で参照すること（`_fetch` と同じ作法）、`false` はテキスト補間では空文字列になるため真偽の出し分けには `data-if` を使うこと。
...
- 同梱デモがグローバル変数（`window.username = ...` など）や id 由来の named access に依存して式を解決していたため、仕様に沿った書き方へ修正した。参照するキーを `data-bind` で宣言する（`data-click-bind` / `data-fetch-bind` で後から入るキーは `null` で宣言）、`data-bind="haori"`（JSON ではない不正な値）と `window.haori = {...}`（予約名前空間との衝突）を廃止する、`data-url-param="user"` を仕様どおりの `data-url-param data-url-arg="params"` + `{{params.user}}` にする、といった変更を行っている。対象は `demo/index.html`、`demo/click/` の 6 ページ、`demo/fetch/` の 4 ページ、`demo/if/data-if-false-demo.html`、`demo/row/data-row-demo.html`、`demo/url/data-url-param-demo.html`。
