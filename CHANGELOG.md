...
- `docs/ja/specs.md`: 「初期 `data-bind` からの入力欄復元」「`data-each` で生成された行への値反映」「行操作の共通仕様（`data-{event}-row-*`）」の各節を追加。`data-each-index` の解決位置、`data-form-list` の 0 件収集を明記。
- `docs/ja/guide.md`: 行操作の節へセレクタ指定・`row-remove-empty`・配列操作であることを追記。
...
- `tests/fetch_error_array.test.ts`: フォームの初期スキャンで逆方向同期のパスが 1 つ増えたことにより、負荷時に既定の試行回数では待機が尽きることがあったため、`waitForCondition` の試行回数を広げた（判定内容は変更なし）。
