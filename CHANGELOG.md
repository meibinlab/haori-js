...
- `docs/ja/specs.md`: 「demo ランタイムでの通信の正規化」節を追加し、正規化の内容（メソッド・送信データ・`Content-Type`）、対象が `data-runtime="demo"` のみであること、ログとイベントでの確認方法、`before-run` の上書き後にも再適用されること、クエリ化できる body の形式を明記。
...
- `tests/demo-runtime-normalization.test.ts`: `data-{event}-before-run` が `fetchOptions` でメソッドや body を上書きしても、demo ランタイムではクエリ付き GET へ再正規化されること（収集済みの送信データと上書きされた body の両方がクエリへ載る）、`URLSearchParams` の body もクエリへ移ること、クエリ化できない body を破棄して警告すること、`haori:fetchstart` が上書き後のメソッドを `requestedMethod` として報告することを検証。embedded ランタイムでは上書きしたメソッドと body がそのまま送られること、`before-run` が無い場合の正規化が変わらないことを対照として確認（6 件のうち 4 件は修正前に失敗）。
