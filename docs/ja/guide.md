...
### 文字列を加工する（正規表現リテラルが使えます）

式の中には正規表現リテラル（`/…/フラグ`）を書けます。装飾記法の除去のような全置換を 1 つの宣言で書けます。

**記述するHTML**:
```html
<div data-bind='{"content":"[b]重要[/b]なお知らせ"}'>
  <p>{{content.replace(/\[\/?(?:b|red|u|large|small)\]/g, '')}}</p>
  <p>空白を1つに: {{'a   b'.replace(/\s+/g, ' ')}}</p>
</div>
```
...
```html
<p data-if="/^\d+$/.test(code)">コードは数字だけです</p>
<p>{{tags.split(/\s*,\s*/).join(' / ')}}</p>
```

置換対象が固定の文字列だけなら、正規表現を使わず `replaceAll` でも書けます（`replace` に文字列を渡すと最初の 1 件しか置換されません）。

```html
<p>{{content.replaceAll('[b]', '').replaceAll('[/b]', '')}}</p>
```

> **注意**: `{{ ... }}` の中では `}}` を含む正規表現（`/a{2}}/` など）は書けません。プレースホルダの終端が先に見つかり、式が途中で切れます。この場合は `data-derive` などの属性値へ移してください。`/` は値が来る位置でだけ正規表現リテラルの開始になるため、`(price + tax) / 2` のような除算はそのまま書けます。
...
> **行数が多い一覧では `data-each-arg` を指定してください。** 指定すると行の中を `{{r.name}}` の形で参照することになり、Haori.js は「この行は行の値だけで描画が決まる」と静的に判断できます。その結果、並べ替えや一部の行だけの更新で**変わっていない行の再評価を省略**できます。指定しない（`{{name}}` のように直接参照する）場合は、要素データのキーが行スコープへ展開されるため参照先を静的に決められず、省略が働きません。200 行・1 行あたり宣言 31 個の一覧で行を 1 つ動かす操作を実測すると、`data-each-arg` あり 48ms・なし約 700ms でした。
...
固定要素は行ではないので、**中の式は一覧そのものを参照できます**（上の `items.length`）。配列が変わるたびに再評価されるため、件数の見出しや「該当なし」の出し分けをそのまま書けます。

> **0.47.1 まではここが正しく動きませんでした。** 固定要素の中の式が初期の判定のまま固定され、`items` が増えても「商品がありません」が消えませんでした。
...
> **導出値の欄は `readonly` にする**
>
> 上の規則の裏返しとして、利用者が一度編集した欄は明示的な供給を受けるまで評価結果に追従しません。`data-attr-value="{{plan.kind === 'A' ? '100' : '200'}}"` のように他の入力から導出した値を入れる欄で、前提が変わったら常に入れ直したい場合は、その欄を `readonly` にしてください。`readonly` の欄は編集の印が付かず、**フォーカスが当たっていても評価結果に追従します**（下記「他の項目から算出した値を送る（`readonly` / `type="hidden"`）」）。編集も許したい場合は、`data-{event}-fetch` の応答や `data-{event}-reset` を契機にしてください。
...
> **FK セレクトでは「型」も揃える**
>
> 参照キーと書込キーを揃えても、**型が食い違うと厳密比較は必ず偽になります。** 選択肢の `id` は API/JSON 由来の**数値**、`name` 付き select の収集値は**文字列**になりやすいため、次の宣言は選択しても偽のままです。
>
> ```html
> <!-- ❌ 1 === "1" は偽。選択が付かず、空の FK が送信される -->
> <option value="{{opt.id}}" data-attr-selected="{{opt.id === m.material.id}}">
> ```
>
> `data-attr-selected` は評価結果が偽なら選択を外すため、**先頭の `option`（多くは空の「選択」行）へ戻ります**。`required` を付けていなければ送信もブロックされません。開発モードでは、この型の食い違いを属性名・テンプレート・両辺の型を添えて警告します（`{{opt.id === m.material.id && 'selected'}}` のように `&& 追加` が続く書き方も対象です）。
>
> 対処は次のいずれかです。
>
> - **推奨①（`data-attr-selected` を使わない）にする。** `name` 束縛だけにすれば真実源が 1 つになり、型の問題そのものが起きません。行の中でも `data-form-list` の逆方向同期が選択状態を決めます。
> - **`data-value-type="number"` で収集値を数値にする。** select の収集値が数値になるので、`opt.id === m.material.id` が成立します（[表示しない項目を型どおりに送る（`data-value-type`）](#表示しない項目を型どおりに送るdata-value-type)）。
> - **式の中で型を揃える。** `{{String(opt.id) === String(m.material.id)}}` のように両辺を変換します。
...
#### 他の項目から算出した値を送る（`readonly` / `type="hidden"`）

同じフォームの他の項目から算出した値を、そのまま送信データに載せたいことがあります。算出結果を入れる欄は、**`readonly` にするか `type="hidden"` にしてください**。
...
<div data-bind='{"unitPrice":1200,"count":3}'>
  <form id="orderForm">
    ...
    <input name="count" type="number" data-attr-value="{{count}}">

    <!-- 画面にも出す場合 -->
    <input name="total" readonly data-attr-value="{{unitPrice * count}}">

    <!-- 画面に出さない場合 -->
    <input name="totalTax" type="hidden" data-value-type="number"
           data-attr-value="{{Math.floor(unitPrice * count * 0.1)}}">
  </form>
</div>
```

算出のもとになる値は、`value` 属性へ直接書かずに`data-bind`から供給してください。フォームの入力値がバインドデータへ載るのは最初の変更以降のため、`value="1200"`と書いただけでは初期表示の時点で`unitPrice`が未解決参照になり、算出欄が空になります（上記「フォーム入力値はスコープに自動投入されない（重要）」）。

普通の（編集できる）入力欄にすると、**利用者が操作している間は算出結果が反映されません**。宣言バインドは、利用者の入力を勝手に書き換えないよう「フォーカス中の欄」と「編集済みの欄」への再適用を止めるためです（仕様「ユーザー編集と宣言バインドの権威」）。とくに **Tab の移動先が算出欄になっている場合**、入力元を確定した瞬間にその算出欄へフォーカスが移るため、値が古いまま残ります。

`readonly` の欄は利用者が値を変えられないため、この保護の対象外です。フォーカスが当たっていても算出結果に追従します。`type="hidden"` はそもそもフォーカスされません。

- 算出値を数値や真偽値として送りたい場合は、上記の`data-value-type`と併用します。
- `<select>` には`readonly`がありません。算出専用の値は`type="hidden"`の入力欄で送ってください。
- 「既定値を入れておき、利用者が上書きできる」欄にしたい場合は、保護されている現在の挙動が正しい形です。`readonly`にはしないでください。
...
> - 注入先は `data-if` の配下に置いても構いません。表示条件の中で状態を出し分ける形（`data-if` の中の一覧に「読み込み中」「失敗しました」を並べる）がそのまま書けます。
...
> **バリデーション失敗時のスクロールはこの属性が無くても行われます。** 不正な欄が画面外にある場合、ブラウザは検証 UI（バブル）の表示に伴って自分でスクロールします。このときエンジンは、先にスクロールして**止まってから**バブルを出します（逆順だとバブルが画面外の位置に固定され、`scroll-behavior: smooth` のページではメッセージが見えません）。この属性は、サーバーエラー時のスクロールを有効にするために指定してください。
...
| `data-each` の描画確定、`data-if` の再表示、フォームのリセット | `refresh`（未適用なら `init`） |
| Haori が入力欄へ値を書き、値が実際に変わったとき | `refresh`（未適用の要素では呼びません） |
| 要素が DOM から外れたとき（行削除など） | `destroy` |

登録が後（外部ライブラリの読み込みが遅い場合など）でも、`register()` の時点で既に描画済みの要素へ遡って適用されるため、読み込み順を気にする必要はありません。逆に、`<body>` 内のスクリプトで Haori の初期表示より前に登録した場合も、`init` は初期表示の走査が済んでから呼ばれます（`init` が受け取る要素は、描画と初期値の反映が済んだ状態です。ただし `data-each` で選択肢を描画する `<select>` では、フォームの初期値が `init` の後に反映されることがあります。その場合は、値が入った後に `refresh` が呼ばれます）。
...
- 外部ライブラリが生成した DOM は `data-external` で監視対象から外してください。`data-external` の配下で連携が生成した入力（Choices.js の絞り込み欄など）はフォーム値として収集されず、`register()` の置き場所や、`data-external` の要素ごと移動したかどうかでも変わりません。ただしネイティブの submit（`formaction` を含む）はブラウザが名前付きの入力をそのまま送るため、送らせたくない生成入力は `name` を外してください。
- `init` が `data-external` の外に生成した宣言（`{{式}}` や `data-*`）は、初期表示の途中でも後から追加した要素と同じく処理されます。ただし、既にある要素を生成した要素で包んだ場合、その生成要素（中の宣言を含む）は初期表示では処理しません。要素を包んだり移したりするライブラリには `data-external` を併用してください。
