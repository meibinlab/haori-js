...
> **注意（デフォルト動作との関係）**: 遅延後の手続きは元のクリック `event` を参照しないため、Haori 側から `preventDefault()` 等でブラウザのデフォルト動作を抑止することはできません。`<a href="…">` や `type="submit"` のボタンに併用すると、遅延された手続きが走る前にリンク遷移・フォーム送信といったデフォルト動作が先に発生します。これらの要素には併用しないでください（Haori の手続きは要素と種別で動作するため、`event` を参照しないこと自体による不具合はありません）。
