...
登録が後（外部ライブラリの読み込みが遅い場合など）でも、`register()` の時点で既に描画済みの要素へ遡って適用されるため、読み込み順を気にする必要はありません。逆に、`<body>` 内のスクリプトで Haori の初期表示より前に登録した場合も、`init` は初期表示の走査が済んでから呼ばれます（`init` が受け取る要素は、描画と初期値の反映が済んだ状態です。ただし `data-each` で選択肢を描画する `<select>` では、フォームの初期値が `init` の後に反映されることがあります）。
