...
11. `data-click-copy` / `data-click-copy-params` - 別要素へ値をコピー
12. `data-click-refetch` - 再フェッチ
13. `data-click-click` - 別要素のクリック
14. `data-click-open` / `data-click-close` - ダイアログ操作
15. `data-click-dialog` / `data-click-toast` - メッセージ表示
16. `data-click-history` - 履歴への pushState
17. `data-click-redirect` - リダイレクト
...
#### `data-click-copy-params`: コピーするキーを絞り込む

`&` 区切りで指定したキーだけをコピーします。指定されていないキーはコピー先の既存値を保持し、コピー元に存在しない指定キーもそのまま維持します。
...
- `data-click-history-form` でフォームの入力値をクエリに追記できます
