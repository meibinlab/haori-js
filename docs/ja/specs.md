...
- **登録はスクリプトの読み込み順に依存しません。** 未登録の名前は適用を保留し、`register()` の時点で、保留した要素へ遡って適用します（開発モードでは保留を一度だけ警告します）。Haori がまだ走査していない要素（`<body>` 内のスクリプトで登録した時点で、初期スキャンの前にある要素など）には `register()` の時点では適用せず、走査の最後に適用します。そのため `init` は、置き場所によらず描画と初期値の反映の後に呼ばれます。ただし `data-each` で選択肢を描画する `<select>` では、フォームの初期値が `init` の後に反映されることがあります。
...
**detail**:
```typescript
{ version: string }  // ライブラリのバージョン（例: '0.48.1'）
```
