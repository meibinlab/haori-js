...
- 非表示のあいだに外部から `style.display` を書き換えた場合は、**追随結果を書き直して非表示へ戻します**。取り込まないだけでは、`data-if-false` が付いたまま要素が見えてしまうためです（判定の基準は内部状態です）。このとき控えは取り直しません。外部の書き換えは宣言ではないため、表示へ戻したときに復元するのはその時点の宣言です
...
**差分更新で再利用した行にも同じ判定を使います。** 一覧が変わって差分更新を行う場合、キーが一致して再利用した行は、要素データとインデックスが同値なら子孫の再評価を省略できます。ただし**省略できるのは行スコープの名前だけを参照しているテンプレートに限ります**。行の外を参照しているテンプレートは、要素データが同値でも行外データの更新で描画が変わるため、省略すると DOM が古い並びのまま残ります。

行の中で別の `data-each` が行外の配列を参照している構成が該当します。並べ替えでグループの並びが変わると、外側の行（グループ）は要素データが同値のまま位置だけが変わるので、内側の一覧が再評価されなければ古い並びが残ります。
...
5. plain object / array / function を Proxy でラップし、`constructor`、`__proto__`、`prototype` へのアクセスを遮断（**ラップは冪等**。後述）
...
**5 のラップは冪等です。** すでにこの仕組みでラップした値をもう一度ラップの対象にしたときは、**同じ Proxy をそのまま返します**（新しい層を重ねません）。式の評価結果はラップ済みの値なので、それが何らかの経路でバインドデータへ入ると、次の評価でラップの層が 1 つ増えます。層が増えるとプロパティアクセス 1 回あたりのトラップ通過数が層の数だけ増えるため、操作を重ねるほど再評価が遅くなります。遮断の内容は層の数によらず同じなので、重ねる意味はありません。
...
**式の中に正規表現リテラル（`/…/フラグ`）を書けます。** 文字列リテラルや数値リテラルと同じく 1 個のリテラルとして扱い、中身は解釈しません。

```javascript
{{ text.replace(/\[\/?(?:b|red)\]/g, '') }}
{{ /^\d+$/.test(code) ? 'OK' : 'NG' }}
{{ text.split(/\s*,\s*/).join('・') }}
```

- 本体の `\` エスケープと `[…]` 文字クラスを追跡して終端の `/` を判定します。文字クラスの中の `/` は終端になりません（`/[/]/` は 1 個のリテラル）
- 終端の `/` に続く英字はフラグとして読みます（`g` `i` `m` `s` `u` `v` `y` `d`）
- リテラルの中身は[暗黙のオプショナルチェーン](#暗黙のオプショナルチェーン)と計算プロパティ名の検査の対象外です。`/a[0-9]/` の `[` はメンバーアクセスではなく文字クラスとして扱います
- リテラルの中身とフラグは識別子の抽出（[バインドに無い識別子の扱い](#バインドに無い識別子の扱い)、[未解決参照の診断](#未解決参照の診断)）の対象外です。`/g/g` の `g` はバインドキーの参照になりません
- リテラルを基点にしたメンバーアクセスも、他のリテラルと同じく禁止プロパティ名の検査を受けます（`/a/.constructor` と `/a/["constructor"]` は遮断されます）

**`/` が正規表現リテラルの開始になるのは、値が来る位置に限られます。** 直前のトークンが識別子・数値・文字列・正規表現リテラル・`)`・`]`・`}`・後置の `++` / `--` のいずれかであれば除算演算子です。`(a + b) / 2`・`items[0] / count`・`count++ / 2` は従来どおり除算になります。

次の場合は式全体を評価できません（結果は `null` になります）。

- 終端の `/` が見つからない（`{{ text.replace(/abc, '') }}`）: `warn` ログを出力します
- `//` と `/*`（コメントは式の構文に含めません）: `warn` ログを出力します
- 本体に改行を含む: JavaScript の正規表現リテラルとして不正なため、コンパイルに失敗して `error` ログを出力します

**`{{ }}` の中では、`}}` を含む正規表現リテラルは書けません。** プレースホルダの終端が先に見つかるため、式が途中で切れます（`{{ text.replace(/a{2}}/g, '') }}` は書けません）。この場合は `data-derive` などの属性値へ移すか、量指定子の書き方を変えてください。
...
**同じことが配列の要素にも当てはまります。** 収集は**集められた行だけ**を集めるため、収集した行だけで配列を組み立て直すと、収集の対象にならなかった要素が失われます。配列も土台を敷いて、収集した行に対応する要素だけを上書きします。**収集した行に対応しない要素は、元の位置に元の値のまま残します。** 絞り込んだ一覧（`data-each="rules.filter(...)"`）で描かれなかった要素と、行そのものが `data-if` で偽になって収集から外れた要素がこれに当たります（[行の対応付けと `data-each-key`](#行の対応付けと-data-each-key)）。

**土台はフォーム自身のバインドデータに限りません。** 収集したキーをフォーム自身が持たず、**祖先**が持っている場合は、最も近い所有者の値を土台にします。`data-form-arg` の宣言は要りません（詳細は[祖先が所有する値の反映（`data-form-arg` なし）](#祖先が所有する値の反映data-form-arg-なし)）。フォームへ `data-bind` を書かず、一覧やレコードを外側の要素で持つ構成が普通にあるため、ここを見ないと上の「失うと、行の表示が空になったり」がそのまま起きます。
...
**`data-form-list` の行そのものを `data-if` で隠した場合、落ちるのはその行の入力欄の値（上書きしないこと）だけで、配列の要素は残ります**（[行の対応付けと `data-each-key`](#行の対応付けと-data-each-key)の「収集した行に対応しない配列要素」）。隠した行を保存値から外したい場合は、行操作（`data-{event}-row-remove`）で配列から取り除いてください。表示条件だけで配列の件数を変えると、絞り込みの条件が変わるたびにレコードが消えます。
...
- **収集した行に対応しない配列要素**: 元の位置に元の値のまま残します。**収集が配列を行数へ切り詰めることはありません**（[双方向バインディングの自動更新](#双方向バインディングの自動更新)の「同じことが配列の要素にも当てはまります」）。絞り込んだ一覧で描かれなかった要素と、行そのものが `data-if` で偽になって収集から外れた要素が対象です。配列から要素を取り除くのは行操作（`data-{event}-row-remove`）と供給の側で、収集の側ではありません。並びも土台の配列に従うため、配列を先に入れ替えて画面を描き直す前に収集が走っても（`data-{event}-row-prev` の直後など）並びは巻き戻りません。
- **`data-each-key` なし**: リストキーがインデックス由来（`__index_N`）になるため、対応付けは**出現順**と同じになります。行数のずれは解消されますが、どの行が消えたかは判別できません。**絞り込んだ一覧（派生配列）では、この対応は行と配列要素を取り違えます**（画面の 2 行目の値が、配列の 2 番目の要素へ書き込まれます）。派生配列を `data-form-list` で収集する場合は `data-each-key` を宣言してください。宣言が無い場合は開発モードで、`data-each` の式ごとに一度だけ警告します。
...
##### 祖先が所有する値の反映（`data-form-arg` なし）

`data-form-arg` を宣言しないフォームでも、**収集したキーを祖先が持っている**構成は成立します。一覧やレコードを外側の要素で持ち、フォームには `data-bind` を書かない書き方です。
...
上の[祖先が所有するレコードの反映（`data-form-arg`）](#祖先が所有するレコードの反映data-form-arg)のうち、**土台の解決とコピーの解除は同じ規則で働きます**。

- コミット時は**収集したキーごとに、最も近い所有者の値を土台に収集値を重ねます**。入力欄に対応しないフィールド（`id`、表示専用のラベルなど）が残るため、コミット後も `{{row.id}}` を参照できます。土台に使う値の型は問いません（`data-form-list` の収集キーは配列になります）。
- 解決の対象は祖先の `data-bind`（バインドデータ）が持つキーだけです。`data-each` の行データと `data-derive` の派生データは対象外です。同じキーを持つ祖先が複数ある場合は**最も近い祖先**が権威です。
- 双方向コミットの書き込み先は**フォーム自身**のバインドデータです。このコピーは祖先をシャドーしますが、祖先が当該キーを更新したときに解除するため、以降の更新が届かなくなることはありません。解除は**そのキーの値が実際に変わったとき**だけ行います（同じ祖先の別キーだけを更新した場合は解除しません。解除すると利用者が確定した編集を巻き戻します）。
- **土台の解決とコピーの解除は、`<form>` と `data-form` コンテナの両方が対象です。** どちらも双方向コミットの書き込み先になるため、どちらもコピーを作って祖先をシャドーしえます。上の[祖先が所有するレコードの反映（`data-form-arg`）](#祖先が所有するレコードの反映data-form-arg)が `data-form` を対象外とするのは**入力欄への流し込み**についてで、シャドーの解除には当てはまりません。
- リセット（`data-{event}-reset`）は、祖先が所有する値へ戻します。祖先が所有するキーはフォーム自身へ書き戻しません。
- 解除でキーが 1 つも残らない場合、`data-bind` 属性は `{}` になります。属性は in-memory のバインドデータをそのまま映すため（[`data-bind`](#data-bind)）、空のバインドデータは `{}` として現れます。属性が消えるわけではありません。

**入力欄への流し込みは `data-form-arg` を宣言した場合だけです。** `data-form-arg` の無いフォームでは、フォームと祖先のどのキーが対応するかを宣言していないため、初期表示でも祖先の更新でも、フォーム直下の入力欄へ値を流し込むことはしません。上の例で入力欄に値が入るのは、`data-each` の行単位の反映（[`data-each` で生成された行への値反映](#data-each-で生成された行への値反映)）が行っているからです。フォーム直下の入力欄へ祖先の値を出したい場合は `data-form-arg` を宣言するか、`data-attr-value` で明示してください。
...
- **予約キー（`_` 始まり）は宣言の取り込みで落としません。** `_fetch`（[`data-fetch-state` / `data-{event}-fetch-state`](#data-fetch-state--data-event-fetch-state)）や `_poll`（[`data-poll-state`](#data-poll-state)）は Haori が実行時に注入する値で、`data-bind` 属性へは書き出しません。属性に現れない以上、宣言を取り込む側も、外部から属性を書き換える側も、この値を運びようがありません。したがって宣言データを差し替えるとき（走査での属性の再適用、外部からの書き換え、`Haori.Core.setBindingData()`、属性の除去）は、新しいデータが同じキーを持たない限り予約キーを引き継ぎます。上の「バインディングデータは空になります」は宣言データについての記述です。取り除きたい場合はキー自体を持たせてください（`{_fetch: undefined}`）。
- **予約キーは宣言データとして外へ出しません。** `data-bind` 属性へのミラー、[`data-{event}-copy`](#data-event-copy) のコピー元、[`data-store`](#data-store) の保存対象のいずれからも除きます。収集（`Form.getValues()`）は DOM の入力欄から行うため、送信ペイロードにも含まれません。
...
- `data-each-array`: 行操作（`data-{event}-row-*`）の書き戻し先の配列。`data-each` に派生配列（`rules.filter(...)` など）を書いたコンテナで行操作を使う場合に宣言します（[派生配列の書き戻し先（`data-each-array`）](#派生配列の書き戻し先data-each-array)）
- `data-each-before`: ループ前に表示する要素をマーク（**ループコンテナの直接の子**に置きます）。固定要素とコンテナ直下のテキストノードは行ではないため、**コンテナのスコープで、バインドデータの更新のたびに再評価します**（行の中の式は行スコープで評価されるのに対し、固定要素の中の式は一覧そのものを参照できます。`data-each-after` へ `data-if="items.length === 0"` を書いて「該当なし」を出す形がこれにあたります）
...
- `data-each-done`: 全行の描画が安定して完了したときに **Haori が自動付与**するマーカー（手動指定不可）。新しい描画サイクルの開始時に外され、完了時に再付与されます。E2E テスト等で `[data-each-done]` の出現を待って描画完了を検知できます。**発火保証**: 初回描画・再 fetch・再バインドなど描画サイクルが走るたびに、コンテナ単位で「除去 → 再付与」が必ず一度行われます。差分更新で実際の DOM 変更がない場合でも、サイクルが安定した時点で再付与されます。これにより外部ウィジェットの再同期契機として利用できます。**非表示のあいだは外れます。** `data-if` が偽のコンテナでは描画を保留するため（[data-if の動作](#data-if-の動作)の「配下は再評価しない」）、保留した時点でマーカーを外し、表示へ戻って描き終えた時点で再付与します。付けたままにすると、非表示のあいだに配列が変わった場合に**古い行数のまま「描画完了」を示し続けます**。**markup が持ち込んだ `data-each-done` は宣言として取り込みません。** このマーカーは判定の結果（追随結果）であって宣言ではないため、Haori が出力した HTML を保存して再配信する構成、`innerHTML` でコピーした断片、`data-import` で取り込む断片に残っていても、フラグメントを作る時点で落とします（[data-if の動作](#data-if-の動作)の「追随結果は**宣言として取り込みません**」と同じ扱い）。落とさないと、行を 1 つも描いていない状態で完了を示します
...
- 配下で、Haori が走査した時点に無かった DOM（外部ライブラリが生成・追加した要素）は、フォーム値の収集を含め Haori の管理に入らない。走査の時点にあった要素（書いた `<select>` など）と、Haori 自身が配下へ描画した要素（`data-each` の `<option>` など）は従来どおり扱う
- この扱いは、`Haori.enhancers.register()` を置く場所や、`data-external` の要素を外して付け直したかどうかでは変わらない。付け直した場合も、配下は走査した時点の構成のまま扱う（外している間に配下を書き換えても取り込まない）
- `data-external` の要素ごと移動しても、配下の書いた要素は移動の後も収集する（連携には [`data-enhance`](#data-enhance) の契機の表どおり `destroy` の後に `init` が呼ばれる）
- Haori の走査より前に生成された DOM は、書いたマークアップと区別できない。生成物を管理から外すには、ライブラリを [`data-enhance`](#data-enhance) で適用する（読み込み中にスクリプトから直接初期化しない）
- ネイティブの submit・`formaction` による送信は Haori を通らないため、ブラウザが配下の名前付きの入力をそのまま送る
...
| フォームのリセット（`data-{event}-reset` とネイティブの `form.reset()` の両方） | リセットした要素の配下 | `refresh`（未適用なら `init`） |
| Haori が入力欄へ値を書き、値が実際に変わったとき（フォームの初期値の反映、宣言バインドやフェッチ応答の反映など） | その入力欄 | `refresh`（未適用の要素では呼びません） |
| 要素が DOM から外れたとき（行削除など） | 外れた部分の配下 | `destroy` |

**リセットの再同期は、値の復元が終わってから 1 度だけ呼びます。** `reset` イベントは値が戻る**前**に発火するため、そのまま再同期すると戻る前の値を読みます。`data-{event}-reset` はリセットの全段（入力欄の復元・バインドデータの更新・再評価）が終わった時点、ネイティブのリセットは値の復元が終わった時点です。`data-{event}-reset` は内部でネイティブのリセットも起こしますが、再同期はどちらか一方だけが行います。フォームの外にある入力（`data-form` コンテナなど）も同じ契機で再同期します。
...
- **登録はスクリプトの読み込み順に依存しません。** 未登録の名前は適用を保留し、`register()` の時点で、保留した要素へ遡って適用します（開発モードでは保留を一度だけ警告します）。Haori がまだ走査していない要素（`<body>` 内のスクリプトで登録した時点で、初期スキャンの前にある要素など）には `register()` の時点では適用せず、走査の最後に適用します。そのため `init` は、置き場所によらず描画と初期値の反映の後に呼ばれます。ただし `data-each` で選択肢を描画する `<select>` では、`init` は描画の確定で呼ばれるため、フォームの初期値の反映が `init` の後になります。**その場合は、値が入った後に `refresh` を呼びます**（上の契機の表）。連携が `init` の時点の選択を自分の管理へ取り込む実装（`<option>` を移すものなど）でも、取り直せます。
- `init` / `refresh` / `destroy` の例外は `error` ログに記録して続行します。1 つの連携の失敗で描画や他の要素の適用を止めません。
- 連携の呼び出し（`init` / `refresh` / `destroy`）が `data-external` の外で起こした DOM の変更（要素の生成、`{{式}}` や `data-*` を含む宣言の追加など）は、初期スキャンの途中でも、後から追加されたノードと同じく取り込みます。取り込んだ宣言の処理は、初期化の完了（[`data-haori-ready`](#data-haori-ready-属性) の付与）より前に終わります。ただし、既にある要素の取り外しと、既にある要素を生成した要素で包むことは、初期スキャンの途中では取り込みません（DOM を動かすライブラリを `data-external` なしで使った場合でも、初期表示で要素を失ったり値を二重に収集したりしないため）。既にある要素を包んだ生成要素は、その中の宣言も含めて初期表示では処理しません。
...
- `data-external` の要素ごと移動した場合も、「削除 → 追加」として観測されるため `destroy` の後に `init` が呼ばれます。配下の書いた要素は移動の後も収集されます（生成 DOM は収集されません）。
...
- `new` の呼び出しが `data-external` の外で起こした DOM の変更は、`data-enhance` と同じく、初期スキャンの途中でも後から追加されたノードと同じく取り込みます。
...
**markup が持ち込んだ `data-importing` は宣言として取り込みません。** この属性は進行状態の表示（追随結果）であって宣言ではないため、フラグメントを作る時点で落とします（[data-if の動作](#data-if-の動作)の「追随結果は**宣言として取り込みません**」と同じ扱い）。落とさないと、上の例のような CSS を当てている画面で、読み込み中の見た目のまま恒久的に固定されます。
...
`data-poll-bind` は既定でバインド先を全置換するため、注入先が bind 先と同じ要素になる構成では `data-poll-bind-merge` を併用してください（併用しない場合も `_poll` は保持されますが、bind 先の他のキーは保持されません）。
...
**評価結果の扱い**: 属性値には `{{...}}` を書けます。**単体プレースホルダの評価結果が `null` / `undefined` / `false` / 空文字 / `0`、または未解決参照になった場合は、確認を出さずに手続きを続けます**（属性が無いものとして扱います）。falsy を未実行として扱う点は副作用属性の規則（[プレースホルダ解決規則](#プレースホルダ解決規則)）と同じです。これにより「破棄されるものがあるときだけ確認する」構成を 1 つのボタンで書けます。

それ以外の評価結果は**文字列にしてメッセージにします**（数値などを「確認しない」側へ落とすと、確認したい場面で確認が黙って飛ぶためです）。配列やオブジェクトは JavaScript の文字列化に従います（オブジェクトは `[object Object]` になります）。`
` 表記は改行へ復元します。
...
<button
  ...
  data-click-redirect="/apply">申込入力へ進む</button>
```
...
**評価結果の扱い**: 属性値には `{{...}}` を書けます。**評価結果が falsy（`null` / `undefined` / `false` / 空文字 / `0`）のときは、属性が無いものとして扱います**（デフォルトの `GET` になります）。それ以外の評価結果は**文字列にしてメソッドにします**（数値などを falsy 側へ落とすと、`POST` を要求したつもりの送信が黙って `GET` になります）。falsy と文字列化の扱いは [`data-{event}-confirm`](#data-event-confirm) と同じです。未解決参照を含む場合は、この属性を含む評価サイクルの `data-{event}-fetch` を実行しません（[プレースホルダ解決規則](#プレースホルダ解決規則)）。

メソッドの綴りは宣言のまま送信します。**送信経路の判定（body を持つか、クエリへ載せるか）と Content-Type の既定値の判定は、大文字へ揃えてから行います**（`post` と `POST` は同じ扱いです）。

```html
<!-- 条件でメソッドを切り替える。新規なら POST、更新なら PUT -->
<button
...
**評価結果の扱い**: 属性値には `{{...}}` を書けます。**評価結果が falsy（`null` / `undefined` / `false` / 空文字 / `0`）のときは、属性が無いものとして扱います**（上記のデフォルト値になります）。それ以外の評価結果は文字列にしてヘッダーの値にします。falsy と文字列化の扱いは [`data-{event}-fetch-method`](#data-event-fetch-method) と同じです。
...
- `_fetch` は内部バインディングデータにのみ設定し、`data-bind` 属性へは書き出しません（`bindchange` イベントも発火しません）。注入先要素の再評価（`data-if` 等）は実行されます。収集は DOM の入力欄から行うため、送信ペイロードにも含まれません。
- 注入先は宣言の取り込みが再び走る位置（`data-if` の配下など）に置けます。`_fetch` は予約キーとして引き継がれるため、取り込みで消えることはありません（[`data-bind`](#data-bind)）。
...
- **配列の所有者**: `data-each` の式を単純な識別子パス（`contracts` / `form.contracts` など）とみなし、根の識別子を持つ最も近い祖先（自身を含む）のバインディングデータを所有者とします。関数呼び出しや演算を含む式（`items.filter(...)` など）は書き戻し先を一意に決められないため、エラーログを出して何もしません。**エラーログには `data-each-array` の宣言を案内します**（下記）。
...
- **派生配列**: `data-each` に関数呼び出しや演算を含む式を書いたコンテナでは、`data-each-array` で書き戻し先の配列を宣言すれば行操作を使えます（[派生配列の書き戻し先（`data-each-array`）](#派生配列の書き戻し先data-each-array)）。
...
##### 派生配列の書き戻し先（`data-each-array`）

`data-each` に派生配列（`rules.filter(...)` など）を書いたコンテナで行操作を使う場合は、`data-each-array` に**書き戻し先の配列**を宣言します。派生配列は結果の位置と元の配列の位置の対応が式によって決まるため、宣言が無いと書き戻し先を決められません（宣言が無い場合はエラーログを出して何もしません）。

- **値は単純な識別子パス**です（`rules` / `form.rules` / 行スコープ名を根に持つ `g.rules`）。所有者の解決は `data-each` と同じ規則で、関数呼び出しや演算を含む式は指定できません。
- **`data-each-key` の指定が必須です。** 表示の並びと配列の並びは一致しないため、行と要素はキーでしか対応付けられません。指定が無い場合はエラーログを出して何もしません。行と要素の対応付けは[編集可能な行への書き込み](#編集可能な行への書き込み)と同じ規則で、キーに一致する要素が無い行では何もしません。
  - **キーの値が重複していても動作します。** 未保存の行が `id: null` のまま複数並ぶ場合など、同じキーの行が複数あるときは、同一キーの中の**出現順**で行と要素を対応付けます（仕様「`data-each`」の重複キーの規則と同じ）。ただしキーによる行の識別は働かないため、重複を検出すると開発モードで警告します。
- **行操作の後、表示の並びは書き戻し先の配列と `data-each` の式から導かれる並びになります。** 移動でグループの初出順が変わる場合は、`data-derive` で作ったグループ見出しの並びも変わります。
- `data-{event}-row-prev` / `-row-next` は、対象の行を**表示上の前後の行が居る位置**へ移します。配列の上で隣り合っていない要素どうしでも、表示された並びのとおりに入れ替わります。表示に出ていない要素（派生で除かれた要素）の順序は変わりません。表示の端（グループの先頭・末尾）では何もしません。
- `data-{event}-row-remove` は対象の行に対応する要素を削除します。最後の 1 行を残す判定は**書き戻し先の配列の件数**で行うため、表示が 1 行だけでも配列に 2 件以上あれば削除します。
- `data-{event}-row-add` は対象の行に対応する要素の**直後**へ空のオブジェクト（`{}`）を挿入します。挿入した要素が派生の条件を満たさない場合、配列には入りますが**表示には出ません**。セレクタでコンテナを指定した追加で、絞り込みの結果が 0 件のときは、表示から位置を決められないため**宣言した配列の末尾**へ追加します。
- `data-each` の式が並びを変える場合（`sort` など）、入れ替えても派生の結果が同じ並びに戻るため、**見た目には反映されません**。並び順そのものを持たせたい一覧では、並び替えの対象になる配列を `data-each` に直接書いてください。
- **`data-form-list` と併用する場合は `data-each-key` を宣言してください。** フォーム値の収集は描かれた行だけを集めますが、収集した行に対応しない配列要素は元の位置に残るため、**バインドデータ（保存値）**から派生で除かれた要素が失われることはありません（[行の対応付けと `data-each-key`](#行の対応付けと-data-each-key)の「収集した行に対応しない配列要素」）。`data-each-key` が無いと行と配列要素を出現順で対応付けるため、値を取り違えます。この扱いは `data-each-array` の有無によらない `data-form-list` の動作で、`data-each-array` は行操作の書き戻し先を決めるだけで収集の対象は変えません。

  **ただし送信データ（`data-{event}-form` が集める payload）には画面にある行だけが入ります。** 送信データは「画面と一致させる」決まりなので（[編集可能な行への書き込み](#編集可能な行への書き込み)）、絞り込みで除かれた行は入りません。payload をそのまま配列の置き換えとして保存すると、除かれた要素がサーバ側で消えます。絞り込んだ一覧を保存する場合は、payload ではなくバインドデータを送ってください。`data-{event}-data` はフォームの収集値より**後**に重なるため、同じキーを上書きできます。

  ```html
  <!-- 送信データの rules を、絞り込む前の配列（バインドデータ）で置き換える -->
  ...
  **配列は祖先の `data-bind` が持っていても構いません。** 収集値はフォーム自身のバインドへ入って祖先を隠しますが、重ね合わせの土台は最も近い所有者から解決し、祖先が配列を更新したときはこのコピーを解除します（[祖先が所有する値の反映（`data-form-arg` なし）](#祖先が所有する値の反映data-form-arg-なし)）。`<form>` と `data-form` コンテナのどちらでも同じです。

`data-derive` でグループ見出しを作り、行をグループごとに絞り込む構成では次のように書きます。
...
**評価結果の扱い**: 評価結果が falsy（`null` / `undefined` / `false` / 空文字 / `0`）または未解決参照のときは**表示しません**。それ以外は文字列にして表示します（[`data-{event}-confirm`](#data-event-confirm) と同じ扱いです）。
...
**detail**:
```typescript
{ version: string }  // ライブラリのバージョン（例: '0.49.0'）
```
...
`snapshot()` は要素ごとの生の集計なので、**どの宣言が時間を使ったか**を知るには集計する側でまとめ直す必要があります。`report(limit?)` は所要時間の合計が大きい宣言から順に並べた要約を返し、同じ内容を表形式でコンソールへ出力します。`limit` は出力する宣言の数で、既定は 20 です。

```javascript
window.__HAORI_EVALUATION_PROFILE__.reset()
window.__HAORI_EVALUATION_PROFILE__.start()
// 遅い操作を 1 回行う（ボタンを 1 回押す）
window.__HAORI_EVALUATION_PROFILE__.report()
```

**集計の単位は「宣言」であり、要素ではありません。** `data-each` の行テンプレートに書いた 1 つの宣言は行の数だけ要素に現れますが、`report()` はそれらを 1 行にまとめます。まとめないと、同じ宣言が行の数だけ別々に並び、いずれも `calls` が 1 になって「どの宣言が重いか」が読めません。同じ位置（属性名・テキストの子の位置）で同じテンプレートの宣言を 1 つのまとまりとして扱います。

戻り値は次の形です。

| キー | 内容 |
| ---- | ---- |
| `totalDurationMs` | 集計期間の評価所要時間の合計 |
| `totalCalls` | 評価回数の合計 |
| `entryCount` | まとめた後の宣言の数 |
| `top` | 所要時間の合計が大きい順の宣言。`limit` 件まで |

`top` の各要素は次の形です。

| キー | 内容 |
| ---- | ---- |
| `where` | 宣言の位置。属性は `要素識別子 @属性名`、テキストは `要素識別子 #子の位置`。まとまりの代表として、所要時間の合計が最も大きい要素の位置を載せる |
| `template` | 宣言の元テンプレート（`{{...}}` を含む生の文字列） |
| `elements` | このまとまりに含まれる要素の数（`data-each` の行なら行の数） |
| `calls` | 評価回数の合計 |
| `totalDurationMs` | 所要時間の合計 |
| `maxDurationMs` | 1 回の最大所要時間 |
| `sharePercent` | `totalDurationMs` が全体に占める割合（小数第 1 位まで） |

**`calls` が `elements` を大きく超える場合は、同じ宣言が 1 回の操作で何度も再評価されています。** 1 要素あたり 1 回を超える評価が起きているときは、行スコープの外を参照している宣言（[`data-each`](#data-each) の行テンプレート）か、再評価を誘発する宣言の連鎖を疑ってください。

**`totalDurationMs` が体感の待ち時間より大幅に小さい場合、原因は式評価ではありません。** この集計が測るのは式の評価だけです。DOM への書き込み、フォームの値の収集、通信の待ち、他ライブラリの処理は含みません。
