...
5. フォーム自身のバインドデータを初期 `data-bind` 宣言（宣言がなければ空）へ戻す。自身のバインドデータを持たないフォームは、祖先を参照しているため何もしない
6. 再評価して `data-each` の行と宣言バインドの現在の評価結果を入力欄へ入れ直し、その結果を含むフォーム値でバインドデータを更新する
