...
- **オープンリダイレクト対策（利用側の責務）**: 本機能が付与する戻り先値は常に現在ページの `pathname + search + hash`（スキーム・ホストを含まない同一オリジン相対）であり、外部 URL を埋め込むことはありません。一方、遷移先（ログインページ等）が**この戻り先クエリを使って遷移し直す処理は利用側の実装**です。受け取った値を復元してそのまま `location.href` 等へ渡すと、`//host` 形式（プロトコル相対）などで外部サイトへ誘導される恐れがあるため、利用側は復元後に「先頭が単一の `/` で始まるローカルパスであること」等を検証してから遷移してください。
