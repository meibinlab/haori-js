...
**同期する値の権威はバインドの評価結果です。** `name` を持つ編集可能な入力へ `data-attr-value` を併用すると、フォームの双方向バインディングと真実源が二つになり、フォーカスが外れた後の再評価で利用者のコミット済み入力が評価結果へ巻き戻ります。参照するキーと `name` が書き込むキーを揃えてください（ガイドの「`name` 付き入力では参照スコープを書込スコープに揃える」を参照）。
