...
- **`data-form-list` との併用は避けてください。** フォーム値の収集は表示中の行だけで配列を組み立てるため、派生で除かれた要素が収集値・保存値から失われます。これは `data-each-array` の有無によらない `data-form-list` の動作です（[行の対応付けと `data-each-key`](#行の対応付けと-data-each-key)の「取得元と収集先が別の配列のとき」）。`data-each-array` は行操作の書き戻し先を決めるだけで、収集の対象は変えません。
